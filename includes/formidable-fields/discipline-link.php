...
/**
 * Custom Discipline Link Formidable field for AcademyXi.
 * 
 * @since 1.0.0
 */
class Field_Discipline_Link
{
    /**
     * Field type
     *
     ...
     * @access private
     */
    protected const TYPE = 'axi_discipline_link';
    ...
    {
        $fields['axi_discipline_link'] = array(
            'name' => esc_html__( 'AXi Discipline Link', 'axi-system' ),
    ...
    function before_field_created( $field_values )
    {
        ...
        {
            $field_values['name'] = esc_html( 'Discipline Link', 'axi-system' );
        }
        return $field_values;
    }
    ...
    {
        if ( ! taxonomy_exists( 'axi_discipline_link' ) )
        {
            return array();
        }
        $data   = [];
        $dlinks = get_terms( array(
            'taxonomy'   => 'axi_discipline_link',
            'hide_empty' => false
        ));
        if ( is_wp_error( $dlinks ) )
        {
            return $data;
        }
        else
